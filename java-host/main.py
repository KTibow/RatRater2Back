    ...
    async with AsyncExitStack() as stack:
        ...
        if decompiler == "cfr" and file_type == ".jar":
            ...
            command = [
                "java",
                "-jar",
                "resources/cfr-0.152.jar",
                str(temp.name),
                "--outputdir",
                temp_dir,
            ]
        elif decompiler == "cfr" and file_type == ".class":
            command = ["java", "-jar", "resources/cfr-0.152.jar", str(temp.name)]
        elif decompiler == "forgeflower":
            command = [
                "java",
                "-jar",
                "resources/forgeflower-2.0.627.2.jar",
                str(temp.name),
                gettempdir(),
            ]
            ...
            command = [
                "java",
                "-jar",
                "resources/quiltflower-1.9.0.jar",
                str(temp.name),
                "--file",
                str(temp_out.name),
            ]
            ...
            command = [
                "java",
                "-jar",
                "resources/procyon-decompiler-0.6.0.jar",
                str(temp.name),
                "-o",
                temp_dir,
            ]
